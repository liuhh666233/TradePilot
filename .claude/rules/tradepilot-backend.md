...
Python 后端，基于 FastAPI + DuckDB，提供行情数据、技术分析、持仓管理等 REST API。

## Key Files

| File | Role |
|------|------|
| `tradepilot/main.py` | FastAPI 入口，挂载路由 + CORS |
| `tradepilot/config.py` | 配置 (DB路径、数据源切换) |
| `tradepilot/db.py` | DuckDB 连接管理 + 10 张表初始化 |
...
main.py (FastAPI app)
  ...
  ├── api/analysis.py    → analysis/ (技术分析引擎)
  └── api/signal.py      → analysis/signal.py (综合信号)
