...
V1 = 市场信息面板 + 交易计划管理（建仓/止损/止盈）

### Dashboard 页面 (4 个区块)

- 区块 1: 大盘指数 K 线 (上半 60%) — 4 指数 tab + K线 + MACD副图 + 成交量 + 信号标注
- 区块 2: 资金面仪表盘 (上半 40%) — 北向资金/ETF资金流/融资余额/市场情绪
- 区块 3: 行业热力图 (下半 60%) — TreeMap (颜色=涨跌幅, 大小=成交额) + 高位预警/低位机会
- 区块 4: 持仓盈亏总览 (下半 40%) — 持仓列表 + 总盈亏 + 抽离提醒

### 交易计划 (trade_plan 表)

三阶段: 建仓 → 止损 → 止盈

- 建仓条件: 技术面(周线金叉/底背离/放量突破) + 估值面(PB/PE低分位/值博率>3) + 资金面(ETF/北向/融资) + 高切低(板块低位)
- 止损条件: 固定比例(默认-10%) + 技术面(周线死叉/破支撑/放量下跌)
- 止盈条件: 固定比例(默认30%) + 技术面(周线死叉/顶背离/高位缩量) + 抽离纪律(情绪过热/板块高位)

状态流转: planning → active → completed/cancelled
...
### Phase 1: 基础骨架 ✅ 已完成
- FastAPI + DuckDB + Mock Provider + API + 前端路由骨架

### Phase 2 (V1): 分析引擎 + 交易计划 + 前端页面
- Step 1: 后端分析引擎 (technical/valuation/fund_flow/sector_rotation/signal/risk)
- Step 2: 交易计划后端 (trade_plan 表 + API)
- Step 3: API 完善 (analysis/signal 接入引擎)
- Step 4: 前端页面 (Dashboard/TradePlan/StockAnalysis/SectorMap/Portfolio)
