    ...
    if len(recent) >= 10:
        ...
        if rrr > 3:
            result["signals"].append(
                {"type": "high_rrr", "name": f"高值博率({rrr})", "direction": "buy"}
            )

    if pe_pct < 30:
        result["signals"].append(
            {"type": "low_pe", "name": f"PE低分位({pe_pct:.0f}%)", "direction": "buy"}
        )
    if pb_pct < 30:
        result["signals"].append(
            {"type": "low_pb", "name": f"PB低分位({pb_pct:.0f}%)", "direction": "buy"}
        )
    if pe_pct > 80:
        result["signals"].append(
            {"type": "high_pe", "name": f"PE高分位({pe_pct:.0f}%)", "direction": "sell"}
        )
    if pb_pct > 80:
        result["signals"].append(
            {"type": "high_pb", "name": f"PB高分位({pb_pct:.0f}%)", "direction": "sell"}
        )
