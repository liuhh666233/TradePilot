...
def evaluate_stop_loss(
    ...
    if pnl_pct <= stop_loss_pct:
        conditions.append(
            {
                "type": "pct_stop",
                "name": f"亏损{pnl_pct:.1f}%达到止损线{stop_loss_pct}%",
                "triggered": True,
            }
        )
        triggered = True
    ...
    if len(kline_df) >= 30:
        ...
        for c in crosses:
            if c["type"] == "death_cross":
                conditions.append(
                    {"type": "death_cross", "name": "MACD死叉", "triggered": True}
                )
                triggered = True
        ...
        if last["volume"] > vol_ma5 * 2 and last["close"] < last["open"]:
            conditions.append(
                {"type": "volume_drop", "name": "放量下跌", "triggered": True}
            )
            triggered = True
...
def evaluate_take_profit(
    ...
    if pnl_pct >= take_profit_pct:
        conditions.append(
            {
                "type": "pct_profit",
                "name": f"盈利{pnl_pct:.1f}%达到止盈线{take_profit_pct}%",
                "triggered": True,
            }
        )
        triggered = True
    ...
    if len(kline_df) >= 30:
        ...
        for d in divergences:
            if d["type"] == "bear_divergence":
                conditions.append(
                    {"type": "bear_divergence", "name": "顶背离", "triggered": True}
                )
                triggered = True
        ...
        if last["volume"] < vol_ma5 * 0.5 and position > 0.8:
            conditions.append(
                {"type": "high_shrink", "name": "高位缩量", "triggered": True}
            )
            triggered = True
    ...
    if market_sentiment and market_sentiment.get("score", 0) >= 80:
        conditions.append(
            {
                "type": "overheated",
                "name": f"市场情绪过热({market_sentiment['score']:.0f})",
                "triggered": True,
            }
        )
        triggered = True
    ...
    if sector_position == "high":
        conditions.append(
            {"type": "sector_high", "name": "所在板块处于高位", "triggered": True}
        )
        triggered = True
