...
class MockProvider(DataProvider):
    ...
    def get_index_daily(
        ...
        dates = pd.bdate_range(start_date, end_date)
        base = {"000001": 3200, "399001": 10500, "399006": 2200, "000688": 1000}.get(
            index_code, 3000
        )
        df = _gen_ohlcv(dates, base)
                ...
                rows.append(
                    {
                        "date": d,
                        "stock_code": code,
                        "margin_balance": balance[i],
                        "margin_buy": _RNG.uniform(0, 1e8),
                    }
                )
        ...
        return pd.DataFrame(
            {
                ...
                "pe_ttm": _RNG.uniform(10, 80, n),
                "pb": _RNG.uniform(1, 15, n),
                "ps": _RNG.uniform(2, 30, n),
                "market_cap": _RNG.uniform(1e10, 1e12, n),
            }
        )
                ...
                rows.append(
                    {
                        "date": d,
                        "sector": sector,
                        "avg_pe": _RNG.uniform(10, 60),
                        "avg_pb": _RNG.uniform(1, 8),
                        "change_1d": _RNG.normal(0, 2),
                        "change_5d": _RNG.normal(0, 4),
                        "change_20d": _RNG.normal(0, 8),
                        "change_60d": _RNG.normal(0, 15),
                    }
                )
        return pd.DataFrame(rows)
